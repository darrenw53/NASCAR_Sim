...
def _build_fanduel_upload_csv(
    ...
    """Build FanDuel upload CSV bytes for top-N unique lineups.

    Output columns match FanDuel template (5 columns labeled 'Driver').
    Each cell is 'FD_ID:Driver Name' (FD_ID already includes slate prefix if provided by FanDuel CSV).
    """
    lineups = optimize_top_n_lineups(
        opt_df,
        n=int(top_n),
    ...
    for _, r in lineups.iterrows():
        lineup_cells = []
        for j in range(1, int(lineup_size) + 1):
            name = str(r.get(f"driver_{j}", "")).strip()
            fdid = str(r.get(f"fd_id_{j}", "")).strip()
            if fdid and fdid.lower() != "nan":
                lineup_cells.append(f"{fdid}:{name}")
            else:
                lineup_cells.append(name)
        rows.append(lineup_cells)

    upload_df = pd.DataFrame(rows, columns=["Driver"] * int(lineup_size))
...
def main():
    ...
    # FanDuel lineup export (Top 150 unique lineups in upload-template style)
    st.download_button(
        "Export Top 150 FanDuel Lineups (upload-template CSV)",
        data=_build_fanduel_upload_csv(
            opt_df,
            salary_cap=int(salary_cap),
            lineup_size=int(lineup_size),
            pool_size=int(optimizer_pool),
