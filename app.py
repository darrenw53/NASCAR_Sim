...
from src.fantasy.optimizer import optimize_lineup, optimize_lineups
...
def load_fd_pool_from_upload(uploaded_file) -> pd.DataFrame:
    """Load FanDuel CSV from a Streamlit UploadedFile."""
    if uploaded_file is None:
        return pd.DataFrame()
    try:
        return pd.read_csv(uploaded_file)
    except Exception:
        # Some FD downloads include extra header rows; try skipping non-data rows
        uploaded_file.seek(0)
        return pd.read_csv(uploaded_file, skiprows=1)


def choose_fd_pool() -> tuple[pd.DataFrame, str]:
    """UI + robust loading for FanDuel player pool."""
    DATA_FD.mkdir(parents=True, exist_ok=True)

    existing = sorted([p.name for p in DATA_FD.glob("*.csv")])
    choices = ["Upload CSV…"] + existing

    choice = st.selectbox(
        "FanDuel slate CSV",
        choices,
        index=0,
        help="Upload the current FanDuel driver salary file, or pick one already in data/fan_duel/.",
    )

    if choice == "Upload CSV…":
        up = st.file_uploader("Upload FanDuel CSV", type=["csv"], accept_multiple_files=False)
        df = load_fd_pool_from_upload(up)
        label = up.name if up is not None else "uploaded"
        return df, label

    # Load from repo folder
    p = DATA_FD / choice
    if not p.exists():
        return pd.DataFrame(), choice
    return load_fanduel_csv(p), choice
...
st.header("Optimizer")
    ...
    fd_df = fd_df if 'fd_df' in locals() else pd.DataFrame()
    ...
    st.divider()
    st.subheader("FanDuel optimizer (multi-lineup, ranked by upside)")

    st.caption(
        "Select how many *unique* lineups to generate. Lineups are ranked by upside (sum of P90 sim FD points when available)."
    )

    lineup_count = st.selectbox("Number of unique lineups", options=[1, 25, 50, 100, 150], index=0)
    ...
    try:
        ...
        show_cols = ["rank", "total_salary", "total_proj", "total_upside"]
        for i in range(1, int(lineup_size) + 1):
            show_cols.append(f"driver_{i}")
        st.dataframe(lineups[show_cols], use_container_width=True)

        st.success(
            f"Generated {len(lineups)} lineup(s) | "
            f"Top lineup — Salary: {int(lineups['total_salary'].iloc[0])} | "
            f"Proj: {lineups['total_proj'].iloc[0]:.2f} | "
            f"Upside: {lineups['total_upside'].iloc[0]:.2f}"
        )

        # --- CSV export (FanDuel upload format) ---
        # FanDuel accepts either "ID + Name" or just "ID". We export "ID:Name" when available.
        if int(lineup_count) >= 25:
            fd_cols = [f"fd_{i}" for i in range(1, int(lineup_size) + 1)]
            drv_cols = [f"driver_{i}" for i in range(1, int(lineup_size) + 1)]

            export_rows = []
            has_fd = all(c in lineups.columns for c in fd_cols)
            for _, r in lineups.iterrows():
                row = []
                for i in range(1, int(lineup_size) + 1):
                    if has_fd and pd.notna(r.get(f"fd_{i}", "")) and str(r.get(f"fd_{i}", "")).strip():
                        row.append(f"{r.get(f'fd_{i}', '')}:{r.get(f'driver_{i}', '')}".strip())
                    else:
                        row.append(str(r.get(f"driver_{i}", "")).strip())
                export_rows.append(row)

            export_df = pd.DataFrame(export_rows, columns=["Driver"] * int(lineup_size))
            csv_bytes = export_df.to_csv(index=False).encode("utf-8")

            st.download_button(
                "Download FanDuel upload CSV",
                data=csv_bytes,
                file_name=f"{race_slug}_fanduel_upload_{int(lineup_count)}.csv",
                mime="text/csv",
